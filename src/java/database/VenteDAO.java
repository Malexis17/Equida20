...
public class VenteDAO {
    ...
    */
    ...
    public static ArrayList<Cheval>  getLesChevaux(Connection connection, String idVente){      
        ...
        {
            //preparation de la requete     
            requete=connection.prepareStatement("SELECT cheval.id, cheval.nom, cheval.vendeur, typecheval.libelle, entraineur.nom AS enom  FROM typecheval, cheval, lot, vente, entraineur WHERE typecheval.id = cheval.id_typeChev AND cheval.id = lot.idCheval AND lot.idVente = vente.id AND cheval.entraineur = entraineur.id AND vente.id = ?");          
            //executer la requete
            requete.setString(1, idVente);
            ...
            while ( rs.next() ) {  
                ...
                unEntraineur.setId(rs.getInt("id"));
                unEntraineur.setNom(rs.getString("enom"));
